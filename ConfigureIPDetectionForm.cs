...
namespace RandM.PDDNS
{
    public partial class ConfigureIPDetectionForm : Form
    {
        ...
        void cmdSave_Click(object sender, EventArgs e)
        {
            ...
            if (lvResults.Items[3].Checked) NewMethodOrder[Index++] = GetExternalIPv4Methods.Http81;
        ...
        void ConfigureForm_Paint(object sender, PaintEventArgs e)
        {
            if (_FirstPaint)
            {
                ...
                ListViewItem LVI = new ListViewItem("UnicastAddress");
                ...
                Application.DoEvents();
                Start = DateTime.Now;
                IP = WebUtils.GetExternalIPv4ByNatPmp();
                End = DateTime.Now;
                if (ExternalIP == IPAddress.None) ExternalIP = IP;
                ...
                Application.DoEvents();
                Start = DateTime.Now;
                IP = WebUtils.GetExternalIPv4ByUpnp();
                End = DateTime.Now;
                if (ExternalIP == IPAddress.None) ExternalIP = IP;
                LVI.SubItems[1].Text = IP.ToString().Replace("255.255.255.255", "Not supported");
                LVI.SubItems[2].Text = ((int)End.Subtract(Start).TotalMilliseconds).ToString() + " milliseconds";

                LVI = new ListViewItem("HTTP(81)");
                LVI.SubItems.Add("Checking...");
                LVI.SubItems.Add("Checking...");
                lvResults.Items.Add(LVI);
                Application.DoEvents();
                Start = DateTime.Now;
                IP = WebUtils.GetExternalIPv4ByHttp(81);
                End = DateTime.Now;
                if (ExternalIP == IPAddress.None) ExternalIP = IP;
                ...
                Application.DoEvents();
                Start = DateTime.Now;
                IP = WebUtils.GetExternalIPv4ByHttp(80);
                End = DateTime.Now;
                if (ExternalIP == IPAddress.None) ExternalIP = IP;
                LVI.SubItems[1].Text = IP.ToString().Replace("255.255.255.255", "randm.ca unreachable");
                LVI.SubItems[2].Text = ((int)End.Subtract(Start).TotalMilliseconds).ToString() + " milliseconds";
